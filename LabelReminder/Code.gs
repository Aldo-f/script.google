...
function parseInterval_(suffix) {
...
function getIntervalFromLabel_(labelName) {
  const parts = labelName.split('/');
  if (parts.length < 2) return null;
  return parseInterval_(parts.slice(1).join('/'));
}
...
function getRemindEveryIntervalLabels_() {
  const allLabels = GmailApp.getUserLabels();
  return allLabels.filter(label => {
    try {
      const name = label.getName();
      if (name === CONFIG.ON_HOLD) return false;
      return name.startsWith(CONFIG.LABEL_PREFIX + '/') && getIntervalFromLabel_(name) !== null;
    } catch (e) {
      log_(`[WARN] Label error: ${e.message}`);
      return false;
    }
  });
}
 ...
 */
function getLastSentByMeDate_(thread) {
 ...
 */
function getLastNonOwnMessage_(thread) {
  const messages = thread.getMessages();
  for (let i = messages.length - 1; i >= 0; i--) {
    const from = messages[i].getFrom();
    if (!from.toLowerCase().includes(CONFIG.MY_EMAIL.toLowerCase())) {
...
function hasRecipientReplied_(thread) {
  const messages = thread.getMessages();
  ...
  for (let i = 1; i < messages.length; i++) {
    ...
    if (isIgnoredSender_(from)) continue;
    return true;
  }
  return false;
}
...
function autoPauseOnReply() {
  const labels = getRemindEveryIntervalLabels_();
  const onHoldLabel = getOrCreateLabel_(CONFIG.ON_HOLD);
  let paused = 0;

  labels.forEach(label => {
    label.getThreads().forEach(thread => {
      if (hasOnHold_(thread)) return;            // al gepauzeerd
      if (!hasRecipientReplied_(thread)) return; // nog geen reply

      thread.addLabel(onHoldLabel);
      paused++;
      log_(`[PAUSED] ${label.getName()} | ${thread.getFirstMessageSubject()}`);
    });
  });

  if (paused > 0) log_(`[AUTO-PAUSE] ${paused} draadje(s) op on-hold gezet wegens reply`);
}
 ...
 */
function getReminderCountFromThread_(thread) {
  const messages = thread.getMessages();
  let count = 0;
...
function getToneFromContext_(reminderCount, firstMessageDate) {
  const now = new Date();
...
function generateReminderText_(originalSubject, originalSnippet, senderName, lang, reminderCount, firstMessageDate) {
  const tone = getToneFromContext_(reminderCount, firstMessageDate);
  ...
  try {
    return cleanAIResponse_(callAI(prompt));
  } catch (err) {
    log_(`[WARN] All AI providers failed (${err.message}), using fallback`);
    return cleanAIResponse_(buildFallbackText_(senderName, originalSubject, lang));
  }
}
...
function buildFallbackText_(senderName, subject, lang) {
...
function findReminderRecipient_(thread) {
  ...
  for (const msg of messages) {
    ...
    if (isIgnoredSender_(from)) continue;
    return {
      email: extractEmail_(from),
      name: extractName_(from) || 'there',
    };
  }
 ...
 */
function extractAllEmails_(header) {
 ...
 */
function createReplyDraft_(toEmail, subject, body, rfcMessageId, threadId) {
...
function sendReminder_({ to, originalSubject, body, thread }) {
  const replyToMsg = thread ? getLastNonOwnMessage_(thread) : null;

  if (replyToMsg) {
    if (isIgnoredSender_(replyToMsg.getFrom())) {
      ...
      const subject = `Re: ${cleanSubject_(originalSubject)}`;

      if (CONFIG.DRY_RUN) {
        log_(`[DRY-RUN] (API reply) → ${to} | ${subject} | thread: ${thread.getId()}`);
        log_(`[DRY-RUN] Body:\n${body}`);
        return;
      }
      ...
      try {
        const rfcMsgId = getRfcMessageId_(replyToMsg.getId());
        createReplyDraft_(to, subject, body, rfcMsgId, thread.getId());
        log_(`${CONFIG.CREATE_DRAFTS ? '[DRAFT]' : '[SENT]'} (API reply) → ${to} | ${subject}`);
        log_(`Body: ${body.replace(/\n/g, ' ').substring(0, 150)}...`);  // Log email body preview for review
      } catch (err) {
        log_(`[WARN] Gmail API failed (${err.message}), fallback to createDraft`);
        GmailApp.createDraft(to, subject, body, { threadId: thread.getId() });
        log_(`[DRAFT] (fallback) → ${to} | ${subject}`);
        log_(`Body: ${body.replace(/\n/g, ' ').substring(0, 150)}...`);  // Log email body preview for review
      }
      return;
    }
    ...
    if (CONFIG.DRY_RUN) {
      log_(`[DRY-RUN] (reply) → ${replyToMsg.getFrom()} | Re: ${originalSubject}`);
      log_(`[DRY-RUN] Body:\n${body}`);
      return;
    }
    if (CONFIG.CREATE_DRAFTS) {
      replyToMsg.createDraftReply(body);
      log_(`[DRAFT] (reply) → ${replyToMsg.getFrom()} | Re: ${originalSubject}`);
      log_(`Body: ${body.replace(/\n/g, ' ').substring(0, 150)}...`);  // Log email body preview for review
    } else {
      replyToMsg.reply(body);
      log_(`[SENT] (reply) → ${replyToMsg.getFrom()} | Re: ${originalSubject}`);
      log_(`Body: ${body.replace(/\n/g, ' ').substring(0, 150)}...`);  // Log email body preview for review
    }
    return;
  }
  ...
  if (CONFIG.CREATE_DRAFTS) {
    ...
    log_(`[DRAFT] → ${to} | ${subject}`);
    log_(`Body: ${body.replace(/\n/g, ' ').substring(0, 150)}...`);  // Log email body preview for review
  } else {
    GmailApp.sendEmail(to, subject, body, options);
    log_(`[SENT] → ${to} | ${subject}`);
    log_(`Body: ${body.replace(/\n/g, ' ').substring(0, 150)}...`);  // Log email body preview for review
  }
}
...
function checkReminders() {
  ...
  const remindLabels = getRemindEveryIntervalLabels_();
  if (remindLabels.length === 0) {
    log_('[OK] Geen remind-every labels gevonden');
    return;
  }
  ...
  remindLabels.forEach(label => {
    const intervalDays = getIntervalFromLabel_(label.getName());
    ...
    const active = threads.filter(t => !hasOnHold_(t));
    if (active.length === 0) return;

    log_(`[CHECK] "${label.getName()}": ${active.length}/${threads.length} actief (${threads.length - active.length} on-hold)`);
    ...
    active.forEach(thread => {
      ...
      if (CONFIG.MAX_REMINDERS > 0 && totalSent >= CONFIG.MAX_REMINDERS) {
        log_(`[LIMIT] Global max reminders (${CONFIG.MAX_REMINDERS}) reached, stopping.`);
        return; // stops processing this label
      }

      const referenceDate = getLastSentByMeDate_(thread);
      const now = new Date();
      ...
      // Taal en body
      const lang = detectLanguage_(sourceMsg.getPlainBody());
      const reminderCount = getReminderCountFromThread_(thread);
      const firstMessageDate = getLastSentByMeDate_(thread);
      const body = generateReminderText_(originalSubject, snippet, recipient.name, lang, reminderCount, firstMessageDate);

      // Versturen (met bronverband voor forward-context)
      sendReminder_({ to: recipient.email, originalSubject, body, thread });
      labelSent++;
      totalSent++;
    });

    log_(`[DONE] ${label.getName()}: ${labelSent} herinnering(en)`);
    // totalSent is already incremented inside active.forEach, do not double-add here
  });

  log_(`[SUMMARY] Totaal: ${totalSent} herinnering(en) verwerkt`);
}
...
function previewReminders() {
  ...
  if (remindLabels.length === 0) {
    log_('Geen remind-every labels gevonden.');
    log_('Maak ze aan in Gmail — bv. "remind-every/2weeks" — en plak ze op een bericht.');
    return;
  }
  ...
  remindLabels.forEach(label => {
    ...
    if (threads.length === 0) {
      log_('  (geen threads met dit label)');
      return;
    }

    threads.forEach(thread => {
      const onHold = hasOnHold_(thread);
      const referenceDate = getLastSentByMeDate_(thread);
      const elapsedDays = (now - referenceDate) / 86400000;
      ...
      log_(`  ${status} | laatst door mij: ${formatDateNL_(referenceDate)} | → ${recipient.email} | ${subject}`);
    });
  });
...
function setup() {
  ...
  // On-hold label maken
  getOrCreateLabel_(CONFIG.ON_HOLD);

  // Intervallabels maken
  examples.forEach(name => getOrCreateLabel_(name));
  ...
  log_('Setup voltooid:');
  log_(`  - Labels: ${examples.join(', ')}`);
  log_(`  - On-hold: ${CONFIG.ON_HOLD}`);
  log_('  - Trigger: checkReminders om de 6 uur');
  log_('');
  log_('Volgende stappen:');
  log_('  1. Voeg GEMINI_API_KEY toe in Project Settings → Script Properties (komma-gescheiden voor meerdere keys)');
  log_('  2. Voeg FREE_LLM_API_KEY toe in Project Settings → Script Properties (fallback provider)');
  log_('  3. Voeg OPENROUTER_API_KEY toe in Project Settings → Script Properties (optionele fallback)');
  log_('  4. Plak een remind-every/* label op een e-mail in Gmail');
  log_('  5. Run previewReminders() om te testen');
  log_('  6. Run dryRun() om drafts te bekijken');
  log_('  7. Zet CONFIG.CREATE_DRAFTS = false om live te gaan');
}
